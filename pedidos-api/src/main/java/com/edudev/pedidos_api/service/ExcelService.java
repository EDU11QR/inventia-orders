...
public class ExcelService {
    ...
    public byte[] generarExcel(List<Pedido> pedidos) {
        try (Workbook workbook = new XSSFWorkbook();
            ...
            // 2. autofiltro nativo sobre las 17 columnas
            ...
            // 3. anchos definidos según el tipo de información
            configurarAnchos(hoja);

            // 4. encabezados en fila 0
            crearEncabezados(hoja, estiloEncabezado);

            // 5. datos desde fila 1
            int numeroFila = 1;
    ...
    private void crearFila(
        ...
        // Col 7: vendedor del pedido (nombre si existe,
        // fallback al propio id)
        escribirTexto(fila, 7, nombreVendedor(pedido), estiloTexto);
        escribirTexto(fila, 8, pedido.getEstado() == null ? null : pedido.getEstado().name(), estiloTexto);
        // Columnas 9 a 14: TOTAL A PAGAR, MEDIO DE COMPRA, MÉTODO DE PAGO,
        // COSTO DE ENVÍO, N. DE OPERACIÓN y ENVÍO/AGENCIA quedan vacías
        escribirTexto(fila, 15, pedido.getMotivoCancelacion(), estiloTextoWrap);
        escribirTexto(fila, 16, pedido.getObservaciones(), estiloTextoWrap);
    }

    private String nombreVendedor(Pedido pedido) {

        if (pedido.getVendedorNombre() != null &&
                !pedido.getVendedorNombre().isBlank()) {

            return pedido.getVendedorNombre();
        }

        return pedido.getVendedorId();
    }
