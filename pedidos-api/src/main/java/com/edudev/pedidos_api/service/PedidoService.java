...
public class PedidoService {
    ...
    // Si se recibe vendedorId, devuelve únicamente los
    // pedidos asociados a ese vendedor
    // =========================================================
    public List<Pedido> listarPedidos(String vendedorId) {
        ...
        return pedidoRepository
                .findByVendedorIdOrderByFechaRegistroDesc(vendedorId);
    }

    // =========================================================
    // LISTAR VENDEDORES PARA EL SELECTOR DE FILTROS
    // =========================================================
    // Devuelve cada vendedor_id con el nombre más reciente
    ...
    public List<VendedorOpcionDTO> listarVendedores() {
        ...
        return pedidoRepository
                .obtenerVendedoresConPedidos()
                .stream()
                .map(projection -> VendedorOpcionDTO.builder()
                        .vendedorId(projection.getVendedorId())
                        .vendedor(projection.getVendedor())
                        .build()
                )
                .toList();
    }
    ...
    // =========================================================
    // EXPORTAR PEDIDOS CON FILTROS ACUMULATIVOS
    // =========================================================
    // Además del rango de fechas aplica los filtros activos
    // en pantalla (vendedor, estado y búsqueda por
    // cliente/producto), de modo que el Excel refleja
    // exactamente lo que el usuario está viendo.
    // Un filtro null o vacío no restringe el resultado
    // =========================================================
    public byte[] exportarPedidosExcel(
            LocalDate fechaInicio,
            LocalDate fechaFin,
        ...
        List<Pedido> filtrados = pedidos.stream()
                .filter(pedido ->
                        coincideVendedor(pedido, vendedorFiltro))
                .filter(pedido ->
                        coincideEstado(pedido, estado))
                .filter(pedido ->
                        coincideBusqueda(pedido, busquedaFiltro))
                .toList();
        ...
        return excelService.generarExcel(filtrados);
    }

    private boolean coincideVendedor(
            Pedido pedido,
            String vendedorId
    ) {
        ...
        return vendedorId.equals(pedido.getVendedorId());
    }
    ...
    private boolean coincideEstado(
        ...
        return pedido.getEstado() == estado;
    }
    ...
    private boolean coincideBusqueda(
        ...
        return (cliente != null &&
               ...
               (producto != null &&
                producto.toLowerCase().contains(busqueda));
    }

    // texto recortado; vacío equivale a sin filtro
